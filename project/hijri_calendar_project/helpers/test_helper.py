    ...
    def assert_invalid_data_response(self, url, invalid_data_dicts):
        ...
        for invalid_dict in invalid_data_dicts:
            if invalid_dict['method'] == 'POST':
                response = self.client.post(url, data=invalid_dict['data'],
                                            format='json')
            elif invalid_dict['method'] == 'GET':
                response = self.client.get(url, data=invalid_dict['data'],
                                           format='json')
            else:
                print('Implement other methods.')  # pragma: no cover
            error_msg = '{}-{}-{}'.format(invalid_dict['label'],
                                          response.status_code,
                                          response.content)
            self.assertEqual(response.status_code, invalid_dict['status'],
                             msg=error_msg)
